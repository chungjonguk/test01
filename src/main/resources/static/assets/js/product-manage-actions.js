...
(function () {
  ...
  function categoryLabel(code) {
    ...
    var opts = window.__CODE_OPTIONS__ && window.__CODE_OPTIONS__.PRODUCT_CATEGORY;
    if (opts) {
  ...
  function renderGrid(products) {
    ...
    grid.querySelectorAll('.btn-delete-product').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        ...
        fetch('/api/ecommerce/products/' + encodeURIComponent(pid), {
          method: 'DELETE',
  ...
  function loadProducts() {
    ...
    fetch('/api/ecommerce/products?' + buildQuery().toString(), {
      headers: { Accept: 'application/json' }
    })
      .then(function (res) {
        if (!res.ok) {
